...
export function init(exitFn) {
    ...
    document.getElementById('settings-ai-enabled').addEventListener('change', (e) => {
        ...
        document.dispatchEvent(
            new CustomEvent('look:ai-enabled-changed', {
                detail: { enabled },
            }),
        );
    });
    ...
    document.getElementById('settings-running-apps').addEventListener('change', (e) => {
        ...
        document.dispatchEvent(
            new CustomEvent('look:running-apps-changed', {
                detail: { enabled },
            }),
        );
    });
    ...
    const BG_LAYOUT_HINTS = {
        center: 'Original size, centered',
        fill: 'Fill area and crop edges',
        stretch: 'Stretch to fill exactly',
        duplicate: 'Repeat as tiles',
    };

    bgLayoutBtn.addEventListener('click', () => {
        bgLayoutMenu.hidden = !bgLayoutMenu.hidden;
    });
    ...
    document.getElementById('settings-save-btn').addEventListener('click', async () => {
        try {
            ...
            updates.lazy_indexing_enabled = document.getElementById('settings-lazy-indexing')
                .checked
                ? 'true'
                : 'false';
            updates.running_apps_placement = document.getElementById('settings-running-apps')
                .checked
                ? 'right'
                : 'none';
...
export function handleKey(e) {
    ...
    if (e.key === 'Escape') {
        e.preventDefault();
        exit(document.getElementById('search-content'), document.getElementById('search-bar'));
        return true;
    }
...
async function loadConfig() {
    try {
        ...
        if (blurActiveItem) {
            blurDD.querySelector('.settings-dropdown-label').textContent =
                blurActiveItem.textContent;
            for (const el of blurDD.querySelector('.settings-dropdown-menu').children)
                el.classList.remove('settings-dropdown-active');
        ...
        document.getElementById('settings-lazy-indexing').checked =
        ...
        const bgLayoutDD = document.getElementById('settings-bg-layout');
        const bgLayoutItem = bgLayoutDD.querySelector(
            `.settings-dropdown-item[data-value="${bgLayout}"]`,
        );
        if (bgLayoutItem) {
            bgLayoutDD.querySelector('.settings-dropdown-label').textContent =
                bgLayoutItem.textContent;
            for (const el of bgLayoutDD.querySelector('.settings-dropdown-menu').children)
                el.classList.remove('settings-dropdown-active');
