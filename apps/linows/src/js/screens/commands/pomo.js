...
export function init() {
    ...
    // Session list toggle
    document
        .getElementById('cmd-pomo-sessions-toggle')
        .addEventListener('click', toggleSessionsList);

    // Add session buttons (stop propagation to avoid triggering toggle)
    document.getElementById('cmd-pomo-add-focus').addEventListener('click', (e) => {
        e.stopPropagation();
        addSession('focus');
    });
    document.getElementById('cmd-pomo-add-break').addEventListener('click', (e) => {
        e.stopPropagation();
        addSession('break');
    });
...
function drawTimer() {
    ...
    const totalSecs =
        activeIndex !== null ? sessions[activeIndex].duration * 60 : sessions[0].duration * 60;
    const progress = activeIndex !== null ? 1 - secondsLeft / totalSecs : 0;
    const color = getTimerColor();

    switch (timerStyle) {
        case 'modern':
            drawModernRing(size, progress, color, totalSecs);
            break;
        case 'vintage':
            drawVintageDial(size, progress, color);
            break;
        case 'minimal':
            drawMinimalText(size, progress, color, totalSecs);
            break;
        default:
            drawModernRing(size, progress, color, totalSecs);
    }
...
function drawModernRing(size, progress, color, totalSecs) {
    ...
    // Time text
    ctx.fillStyle =
        getComputedStyle(document.documentElement).getPropertyValue('--font-color').trim() ||
        '#fff';
...
function musicPlayCurrent() {
    ...
    ipcPlay(musicTracks[musicIndex]).catch((err) => console.error('[music] play error:', err));
    musicPlaying = true;
    startEndPoll();
    updateMusicUI();
}
