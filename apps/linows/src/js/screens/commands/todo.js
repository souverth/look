...
export function init() {
    ...
    daysEl.addEventListener('click', (e) => {
        ...
        switch (act) {
            case 'toggle':
                toggleTask(group, task);
                break;
            case 'del':
                removeTask(group, task);
                break;
            case 'complete-all':
                completeAll(group);
                break;
...
function heatmapHtml(counts, width) {
    ...
    const labels = HEATMAP_ROW_LABELS.map(
        (l) => `<span class="cmd-todo-heat-label">${l}</span>`,
    ).join('');
...
function heatLegendHtml() {
    const swatches = HEAT_LEVEL_OPACITY.map(
        (o) => `<span class="cmd-todo-heat-cell" style="opacity:${o}"></span>`,
    ).join('');
...
function insightsHtml(trend) {
    ...
    return tiles
        .map(
            ([value, label]) => `<div class="cmd-todo-insight">
